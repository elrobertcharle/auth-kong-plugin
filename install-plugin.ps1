...
docker exec --user=root ${ContainerId} apt-get update
docker exec --user=root ${ContainerId} apt-get install zip
docker exec --user=root ${ContainerId} apt-get install unzip
docker exec --user=root ${ContainerId} luarocks install lua-resty-http
...
docker exec  --user=root ${ContainerId} /bin/bash -c  echo "luarocks make"
docker exec  --user=root ${ContainerId} /bin/bash -c "cd /del && luarocks make"
docker exec  --user=root ${ContainerId} /bin/bash -c  echo "luarocks pack"
